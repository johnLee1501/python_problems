...
"""
list_languages = ['C', 'CPP', 'JAVA', 'PYTHON', 'PERL', 'PHP', 'RUBY', 'CSHARP', 'HASKELL', 'CLOJURE', 'BASH', 'SCALA',
                  'ERLANG', 'CLISP', 'LUA', 'BRAINFUCK', 'JAVASCRIPT', 'GO', 'D', 'OCAML', 'R', 'PASCAL', 'SBCL',
                  'DART', 'GROOVY', 'OBJECTIVEC']
N = int(input())
for _ in range(N):
    lenguaje = input().strip().split()[-1]
    print('VALID' if lenguaje in list_languages else 'INVALID')
"""
