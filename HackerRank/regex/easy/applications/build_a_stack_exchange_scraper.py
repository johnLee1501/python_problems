...
results = re.findall(
    r'question-summary-([\d]+)".+?class="question-hyperlink">(.+?)</a>.*?class=\"relativetime\">(.+?)</span>', stack,
    re.DOTALL)
...
list_id = re.findall(r'questions/([\d]+)', fragment)
list_text = re.findall(r'question-hyperlink">([^<]+)', fragment)
list_hour = re.findall(r'relativetime">([^<]+)', fragment)
for x in range(len(list_id)):
    print(list_id[x], list_text[x], list_hour[x], sep=';')"""
