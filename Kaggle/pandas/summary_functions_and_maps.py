...
# How often does each country appear in the dataset? Create a Series reviews_per_country mapping countries to the count
# of reviews of wines from that country.
...
# I'm an economical wine buyer. Which wine is the "best bargain"? Create a variable bargain_wine with the title of the
...
# understand - we'd like to translate them into simple star ratings. A score of 95 or higher counts as 3 stars, a score
# of at least 85 but less than 95 is 2 stars. Any other score is 1 star.
...
def points_starts(row):
    start = 3 if (row.points >= 95 or row.country == 'Canada') else 2 if row.points >= 85 else 1
    return start


star_ratings = reviews.apply(points_starts, axis='columns')
